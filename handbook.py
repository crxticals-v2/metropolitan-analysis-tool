...
def _sep(spacing: int = 1, divider: bool = True) -> dict:
    """Type 14 — Separator. Used on all content/section pages."""
...
def _select_menu(custom_id: str = "hb_select",
                 placeholder: str = "Select a handbook section…") -> dict:
    """
    Type 1 — ActionRow containing a Type 3 StringSelect.
    Builds options from _NAV + _SELECT_DESCRIPTIONS.
    """
    options = [
        {
            "label": label,
            "value": key,
            "description": _SELECT_DESCRIPTIONS.get(key, ""),
        }
...
# Short descriptions shown as sub-text in the select menu options.
_SELECT_DESCRIPTIONS: dict[str, str] = {
    "intro":       "Purpose, jurisdiction, quota & callsign requirements",
    "command":     "Command structure and specialisations",
    "personnel":   "Personnel standards, vehicles, uniforms & equipment",
    "deployment":  "Deployment doctrine and response criteria",
    "tactical":    "Tactical operations, K-9 & protection procedures",
    "majorcrime":  "Major Crimes Section — investigative operations",
    "incident":    "Incident Command framework",
    "logistics":   "Internal logistics and Intel Points",
    "cqb_theory":  "CQB Theory & Fundamentals",
    "cqb_tactics": "CQB Breaching & Maneuvers",
}
...
def build_message(active: str = "menu") -> list:
    ...
    • Menu page  → section content + select-menu dropdown inside the container.
    • Section pages → section content + labelled navigation buttons inside the container.
    """
    ...
    if active == "menu":
        # Replace buttons with a select-menu dropdown on the main menu
        content_children.extend([
            _sep(spacing=2),
            _text("### Browse Sections"),
            _select_menu(),
        ])
    else:
        content_children.extend([
...
async def _ephemeral_respond(interaction: discord.Interaction, components: list) -> None:
    """
    Type-4 interaction callback (CHANNEL_MESSAGE_WITH_SOURCE) with V2 + ephemeral flags.
    Called when a section is chosen from the main-menu select dropdown.
    The message is only visible to the user who interacted.
    """
    url = (
        f"https://discord.com/api/v10/interactions/"
        f"{interaction.id}/{interaction.token}/callback"
    ...
    payload = {
        "type": 4,
        "data": {
            "flags": _V2_FLAG | 64,   # IS_COMPONENTS_V2 | EPHEMERAL
            "components": components,
        },
    }
    async with aiohttp.ClientSession() as session:
...
class HandbookCog(commands.Cog, name="Handbook"):
    ...
    async def on_interaction(self, interaction: discord.Interaction) -> None:
        ...
        • Select menu (component_type 3, custom_id 'hb_select'):
            Sends a new **ephemeral** message in the channel showing the chosen
            section (with navigation buttons so the user can browse further).

        • Buttons (component_type 2, custom_id 'hb_<key>'):
            Updates the existing message in-place (works for both the channel
            message and inside an ephemeral).
        ...
        # ── Select-menu: open the chosen section as an ephemeral message ─────
        if component_type == 3 and custom_id == "hb_select":
            ...
            await _ephemeral_respond(interaction, components)
            return

        # ── Buttons: update whichever message the button lives in ─────────────
